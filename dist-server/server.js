...
if (fs.existsSync(POLICY_FILE)) {
    try {
        ...
        console.error("[server] Failed to parse policies.json:", e);
    }
}
// ─════════════════════════════════════════════════════════════════════════════
//  ACHIEVEMENTS DATA
...
const STATS = {
    ...
    hackathonWins: ACHIEVEMENTS.filter((a) => a.category === "Hackathon" &&
        ["Finalist", "6th Place", "1st", "2nd", "3rd"].some((s) => a.status.includes(s))).length,
...
function cachedResponse(res, body, mime = "text/html") {
...
function serveImage(folder, label) {
    return (req, res) => {
...
app.get("/health", (_req, res) => {
    const countImages = (folder) => fs.existsSync(folder)
...
app.get("/:appName", (req, res, next) => {
    if (getSubdomain(req) !== "privacy-policy")
        return next();
    const appData = policies[req.params.appName];
    if (!appData)
        return res.status(404).send("App policy not found");
    const tp = path.join(TEMPLATES, "privacy_policy.html");
    if (!fs.existsSync(tp))
        return res.status(404).send("Template not found");
    let html = fs.readFileSync(tp, "utf-8");
    html = html.replace(/\{\{\s*app\.(\w+)\s*\}\}/g, (_, k) => escapeHtml(String(appData[k] ?? "")));
    return cachedResponse(res, html);
});
// ─════════════════════════════════════════════════════════════════════════════
